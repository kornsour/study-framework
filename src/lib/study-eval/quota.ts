 ...
 */
export async function reserveAiSlot(ip: string): Promise<AiGate> {
	...
	const [row] = await db
		.insert(aiUsage)
		.values({ bucketKey: ipKey(ip), period, count: 1, tokens: 0 })
		.onConflictDoUpdate({
	...
	if (used > env.AI_FREE_EVALS_PER_MONTH) {
		await refundAiSlot(ip); // roll back the reservation we just made
		const n = env.AI_FREE_EVALS_PER_MONTH;
		return {
			allowed: false,
			reason: "ip-limit",
			message: `You've used your ${n} free AI ${n === 1 ? "analysis" : "analyses"} this month. The full deterministic scorecard below still works; AI assist resets next month.`,
		};
	}
...
/** Give back a reserved IP slot (AI call failed, or reservation exceeded the limit). */
export async function refundAiSlot(ip: string): Promise<void> {
	const period = currentPeriod();
	await db
		.update(aiUsage)
		.set({ count: sql`GREATEST(${aiUsage.count} - 1, 0)`, updatedAt: new Date() })
		.where(and(eq(aiUsage.bucketKey, ipKey(ip)), eq(aiUsage.period, period)));
}

/** Record the tokens a successful AI call actually spent, against the IP and the global cap. */
export async function commitAiTokens(ip: string, tokens: number): Promise<void> {
	const period = currentPeriod();
	// Attribute tokens to the IP row (its count was already incremented on reserve).
	await db
		.update(aiUsage)
